...
// Form controller
app.controller('ResponseManagementCtrl', ['$scope', '$modal','resource','toaster', function ($scope, $modal,resource,toaster) {
    $scope.totalPage = 1;
    $scope.currentPage = 1;
    $scope.pageSize = 10;
    $scope.responses = {};
    ...
    $scope.loadData = function () {
        // ajax请求分页获取列表
        $scope.responses = [];
        resource.get('../back/responses', {
            pn:$scope.currentPage
        }).then(function (result) {
            if (result.code==1) {
                $scope.responses = result.extend.pageInfo.list;
...
app.controller('responseModalCtrl', ['$scope', '$modalInstance','response', function ($scope, $modalInstance, response) {
    $scope.huifu = response;
